            ...
            DialogButton(
                onPressed: () async {
                  ...
                  if (_formKey.currentState!.validate()) {
                    ...
                    BIP32 nodeFromSeed = new BIP32.fromSeed(
                        Uint8List.fromList(HEX.decode(convertedSeed)));
                    final node = nodeFromSeed.derivePath("m/84'/0'/0'/0/0");
                    final address =
                        P2WPKH(data: new PaymentData(pubkey: node.publicKey))
                            .data
                            ?.address;
                    globals.segwitAddress = address ?? "";
                    print(address);
                    final key = ep.Key.fromUtf8(
                        "${md5.convert(utf8.encode(passwordValue.text))}");
